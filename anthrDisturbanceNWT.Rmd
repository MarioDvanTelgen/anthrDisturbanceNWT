...
setPaths(modulePath = file.path("D:/GitHub/ADM/modules"))
...
parameters <- list(
  ...
  lineDensity = list(
    # features = c("Seismic", "Road"), # all = c("Airstrip","Pipeline", "Powerline", "Railway", "Road", "Seismic", "Unknown")
    features = c("Roads"), # all = c("Airstrip","Pipeline", "Powerline", "Railway", "Road", "Seismic", "Unknown") # use "disturbanceX" 'Roads' if lineFeatures are 
    searchRadius = 10000, # meters; 10, 10000 (test, Roads) # sigma in gaussian, xmax in uniform
    resolution = 250,# meters; 1, 250 (test, Roads) # eps (width and height of pixels)
    method = "FFT", # c("FFT", "focal")
    tileDims = rep(2, 2)**9, # c(256,256), 2319.03 @ 1024dims; 2263.36 secs @ 2048dims
    boundaryEffect = "lethal", # only worked with method "FFT".
    noiseRemoval = TRUE, # removes all below minimal density
...
modules <- list("anthrDisturbanceNWT", "lineDensity")
objects <- list()
inputs <- list()
outputs <- list()
