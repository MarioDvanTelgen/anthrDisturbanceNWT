...
#    `functionName` is a function's name in your module (e.g., `anthrDisturbanceNWTEvent1`).
# 2. Copy this file to the tests folder (i.e., `D:/GitHub/modules/anthrDisturbanceNWT/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("anthrDisturbanceNWT")
  path <- list(modulePath = "D:/GitHub/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    anthrDisturbanceNWT = list(.saveInitialTime = NA)
  )
  ...
  if (exists("anthrDisturbanceNWTEvent1", envir = .GlobalEnv)) {
    simOutput <- anthrDisturbanceNWTEvent1(mySim)
  } else {
  ...
  if (exists("anthrDisturbanceNWTEvent2", envir = .GlobalEnv)) {
    simOutput <- anthrDisturbanceNWTEvent2(mySim)
  } else {
